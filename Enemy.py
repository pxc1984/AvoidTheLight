...
class Light(pygame.sprite.Sprite):
    def __init__(self, x, y, enemy: Enemy):
        ...
        for i, color in enumerate(COLORS['saturation_colors']):
            pygame.draw.circle(self.image, color, (enemy.brightness, enemy.brightness), enemy.brightness - i*5)
        pygame.draw.circle(self.image, (255, 255, 255), (enemy.brightness, enemy.brightness), enemy.brightness)
        self.rect = self.image.get_rect(x=x, y=y)

        self.mask_image = pygame.surface.Surface((enemy.brightness*2, enemy.brightness*2))
    ...
    def update(self, surface: pygame.surface.Surface, enemy: Enemy, level: pygame.sprite.Group):
        self.generate_mask(surface, level, enemy)
        self.draw(surface, enemy)
        # self.redraw(surface, level, enemy)

    def draw(self, surface: pygame.surface.Surface, enemy: Enemy):
        self.rect.center = enemy.rect.center
        surface.blit(self.image, self.rect)

# Основной задачей переписывания этой функции можно считать маски, по которым герой должен взаимодействовать с врагом и необходимость в плавном исчезании света
# Реализовываться это всё будет с помощью создания маски, по которой уже и будет рисоваться изображение и проверять коллизию герой
    ...
    def generate_mask(self, surface: pygame.surface.Surface, level: pygame.sprite.Group, enemy: Enemy):
        collided_tiles = []
        for tile in level:
            if pygame.sprite.collide_rect(self, tile):
                collided_points = []
                if self.rect.collidepoint(tile.rect.topright):
                    collided_points.append((tile.rect.topright, round(math.atan2(tile.rect.topright[1] - self.rect.centery, tile.rect.topright[0] - self.rect.centerx), 6)))
                if self.rect.collidepoint(tile.rect.topleft):
                    collided_points.append((tile.rect.topleft, round(math.atan2(tile.rect.topleft[1] - self.rect.centery, tile.rect.topleft[0] - self.rect.centerx), 6)))
                if self.rect.collidepoint(tile.rect.bottomright):
                    collided_points.append((tile.rect.bottomright, round(math.atan2(tile.rect.bottomright[1] - self.rect.centery, tile.rect.bottomright[0] - self.rect.centerx), 6)))
                if self.rect.collidepoint(tile.rect.bottomleft):                        
                    collided_points.append((tile.rect.bottomleft, round(math.atan2(tile.rect.bottomleft[1] - self.rect.centery, tile.rect.bottomleft[0] - self.rect.centerx), 6)))
                collided_tiles.append(collided_points)
        collided_points = []
        exit_values = []
        for points in collided_tiles:
            s = sorted(points, key=lambda x: x[1])
            exit_values.append((s[0], s[-1]))
